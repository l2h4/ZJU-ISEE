...
{
  URV v = ((intRegs_.read(di->op1())<<intRegs_.read(di->op2())) | (intRegs_.read(di->op1())>>(32-intRegs_.read(di->op2()))));
  intRegs_.write(di->op0(), v);
}
...
Hart<URV>::execRotright(const DecodedInst* di)
{
  ...
  URV v = (u_int32_t) ((intRegs_.read(di->op1())>>intRegs_.read(di->op2())) | (intRegs_.read(di->op1()) << rs2));
  intRegs_.write(di->op0(), v);
}
