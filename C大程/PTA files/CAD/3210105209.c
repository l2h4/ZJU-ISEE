...
void Save()
{
	...
	TraverseLinkedList(list[LINE], linesave);
    fclose(fp);
}

void linesave(void *line)
{
	LineT ptr;
	ptr=(LineT)malloc(1000);
	ptr=(LineT) line;
	
	sprintf(ch, "%lf %lf %lf %lf %lf", 1, ptr->x1, ptr->y1, ptr->x2, ptr->y2);
...
void pictureopen()
{
	fp=fopen("picture.txt", "rb+");
	LineT Ln;
	Ln=(LineT)malloc(1000);
	while(!feof(fp)){
