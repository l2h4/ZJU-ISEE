...
版本日志：
Ver_hh:
1、对外接ECG信号进行采样，电压形式，单位为mV，采样频率100Hz
3、通过Uart发送采样ECG数据（格式：V=xxxxmV\r\n）
4、通过nRF905发送采样数据（格式同上）,工作频率为：430MHz + 组号 MHz，功率：10dBm。

【更新部分】
1.检查NRF905有没有收到数据，并获取数据
2.通过UART来发送收到的数据：
...
#define RF_CH			96								// RF射频通道 0~511	(工作频率 = 422.4MHz + RF_CH × 0.1MHz )本组设置为430+2MHz
...
xdata uint	 	n95_freq = 4360-4200;   // 工作频率 0~511 （本小组组号是2，所以设置为430+2=432MHz），频率范围设置成420-470MHz
xdata uchar  	n95_pwr = 3;			// 输出功率	0~3(10,6,-2,-10)（“3”对应的功率是10dBm）
...
ulong	Temp;
ulong   Temp_rx;
xdata uchar Uart_Send_Buff[32];//UART缓冲区域
...
bit		timer_flag_ECG;//ECG采样定时器

/*定义输出格式字符*/
const unsigned char *str1 = "V=";
const unsigned char *str2 = "mV\r\n";
//
...
void	Key_treat()
{
	...
	//频率递减，步距是1MHz，频率范围是423-472Mhz,define:起始频率：n95_freq = 4230-4200=30; 终止频率472MHzn95_freq = 4720-4200=520
	if (Key2_press_flag)
	{
		LED2_ON;
		Delay(100);
...
{
	uchar i=0;
	Uart_Send_Buff[0] = 0xAA;
	Uart_Send_Buff[1] = 0xFF;
	Uart_Send_Buff[2] = 0xF1;
	Uart_Send_Buff[3] = 0x02;
	Uart_Send_Buff[5] = (Data & 0xFF00) >> 8;
	Uart_Send_Buff[4] = Data & 0xFF;
	Uart_Send_Buff[6] = 0;
	Uart_Send_Buff[7] = 0;
//	Uart1_tx_len = 2;
	for(i=0;i<Uart_Send_Buff[3]+4;i++)
	{
		Uart_Send_Buff[6] += Uart_Send_Buff[i];
		Uart_Send_Buff[7] += Uart_Send_Buff[6];
	}

	for (Uart_Send_Count=0;Uart_Send_Count<8;Uart_Send_Count++)
	{
	SBUF0 = Uart_Send_Buff[Uart_Send_Count];
	while(TI0 ==0){}
	TI0 = 0;
...
void main(void)
{
	...
	{
		Key_treat();	
		if (timer_flag_ECG==1)  //ECG采样
		{
			ADC0_Init_ECG();
			AD0BUSY = 1;					//开始ADC转换
			while(AD0BUSY){}//等待转换完毕，AD0BUSY自动清零退出循环
			
			if(n95_rxtx ==1){//设置为发送模式
				
			/*通过串口UART输出*/
				//SendString("Send an ECG signal:");
				//SendString("V=");
				SendUint(Temp);
				//SendString("mV\r\n");
			...
			/*nRF905传输数据*/
				//将字符串数据“V=‘Temp’mV”传输给缓冲区
				index = 0;
				/*
				for(i=0; i<strlen(str1); i++)
				{
					n95_TX_Buff[index++] = str1[i];
				}
				*/
	      ...
	      index++;
			...
			if(n95_rxtx ==0){//判断是否是接收模式
				if(n95_Check_DR(n95_RX_Buff)){								// 检查接收，接收数据在变量n95_RX_Buff中
						index = 0;
						Temp_rx=0;//用于存储接收的ECG数据
						//判断接收到的数据格式：
						/*for(index=0;index<RF_DATA_WIDTH;index++){
							SendUchar(n95_RX_Buff[index]);
							SendString("\n");
						}
						*/
						LED3_ON;//LED3闪烁表示数据发送中
						...
						//接收端数据解析
						if(n95_RX_Buff[0]!=NULL){//表示传输的值不是空值（有接收到对应的信号）
			      ...
			      index++;
						...
						//UART显示接收信号
						//SendString("Recieve an ECG signal:");
						//SendString("V=");
						SendUint(Temp_rx);
						//SendString("mV\r\n");
				...
				/*
			if(n95_Check_DR(n95_RX_Buff)){								// 检查接收，接收数据在变量n95_RX_Buff中
				if(BUFF_Check(n95_TX_Buff,n95_RX_Buff,28)){ 			// 检查缓存,比较接收和发送的数据是否相同
					...
					if(n95_rxtx==1) 		// tx模式下的接收->清除Error条件
					{
						n95_rxtx=1;
						n95_back++;
						LED4_ON;//LED4闪烁表示接收到回发数据
						Delay(1);
						LED4_OFF;
						SendString("Return:");
						SendUint(n95_back);
